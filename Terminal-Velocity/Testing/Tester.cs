...
namespace Testing
{
    public class Tester
    {
        ...
        {
            int x = 0;
            if(args[0].Equals("CTCTester"))
            {
                Form CTCTestWindow = new Form();
                TerminalVelocity.Environment env = new TerminalVelocity.Environment();
                CTCOffice.CTCOffice ctc = new CTCOffice.CTCOffice(env, env.PrimaryTrackControllerRed, env.PrimaryTrackControllerGreen);
                CTCTestWindow.Controls.Add(new CTCOffice.CTCOfficeGUI(env,ctc));
            }
            else
            {
                int totalPass = 0;
